            ...
            def launch_simulator():
                try:
                    if os.name == 'nt':  # Windows
                        ...
                        terminals = ['gnome-terminal', 'xterm', 'konsole', 'x-terminal-emulator', 'terminal']
                        for terminal in terminals:
                            try:
                                ...
                                if terminal == 'gnome-terminal':
                                    ...
                                    subprocess.Popen([
                                        terminal, '-hold', '-e', 'bash', '-lc', command
                                    ])
                                    ...
                                    # Fallback for generic launchers: invoke bash explicitly.
                                    subprocess.Popen([
            ...
            try:
                if os.name == 'nt':
                    ...
                    tell application "Terminal"
                        do script "cd '{simulator_dir}' && python3 simulator_wqb.py; exec bash"
                        activate
                    end tell
                    '''
                    ...
                    for terminal in terminals:
                        try:
                            command = f'cd "{simulator_dir}" && python3 simulator_wqb.py'
                            if terminal == 'gnome-terminal':
                                subprocess.Popen([terminal, '--working-directory', simulator_dir,
                                                '--', 'python3', 'simulator_wqb.py'])
                            elif terminal in ('xterm', 'x-terminal-emulator'):
                                subprocess.Popen([terminal, '-hold', '-e', 'bash', '-lc', command])
                            elif terminal == 'konsole':
                                subprocess.Popen([terminal, '--hold', '-e', 'bash', '-lc', command])
                            else:
        ...
        def run_script():
            try:
                if os.name == 'nt':
                    ...
                    script = f'''
                    tell application "Terminal"
                        do script "cd '{simulator_dir}' && python3 alpha_submitter.py; exec bash"
                        activate
                    end tell
                    '''
                    subprocess.Popen(['osascript', '-e', script], env=submitter_env)
                else:
                    # Linux: Try multiple terminal emulators
                    terminals = ['gnome-terminal', 'xterm', 'konsole', 'x-terminal-emulator']
                    for terminal in terminals:
                        try:
                            command = f'cd "{simulator_dir}" && python3 alpha_submitter.py'
                            if terminal == 'gnome-terminal':
                                subprocess.Popen([terminal, '--working-directory', simulator_dir,
                                                '--', 'python3', 'alpha_submitter.py'], env=submitter_env)
                            elif terminal in ('xterm', 'x-terminal-emulator'):
                                subprocess.Popen([terminal, '-hold', '-e', 'bash', '-lc', command], env=submitter_env)
                            elif terminal == 'konsole':
                                subprocess.Popen([terminal, '--hold', '-e', 'bash', '-lc', command], env=submitter_env)
                            else:
                                subprocess.Popen([terminal, '-e', 'bash', '-lc', command], env=submitter_env)
                            break
                        except FileNotFoundError:
                            continue
                    else:
                        # Fallback: Run in background
                        print("Warning: No terminal emulator found, running in background")
                        subprocess.Popen([sys.executable, 'alpha_submitter.py'], cwd=simulator_dir, env=submitter_env)
            except Exception as e:
        ...
        def run_script():
            try:
                if os.name == 'nt':
                    ...
                    script = f'''
                    tell application "Terminal"
                        do script "cd '{hk_simulator_dir}' && python3 autosimulator.py; exec bash"
                        activate
                    end tell
                    '''
                    subprocess.Popen(['osascript', '-e', script])
                    ...
                    for terminal in terminals:
                        try:
                            command = f'cd "{hk_simulator_dir}" && python3 autosimulator.py'
                            if terminal == 'gnome-terminal':
        ...
        def run_script():
            try:
                if os.name == 'nt':
                    ...
                    for terminal in terminals:
                        try:
                            command = f'cd "{transformer_dir}" && python3 "Transformer.py"'
                            if terminal == 'gnome-terminal':
...
def api_pipeline_create():
    ...
    if missing:
        return jsonify({'ok': False, 'error': f'缺少必要字段: {missing}。请确保已登录BRAIN平台。'}), 400
...
def api_pipeline_start(pipeline_id):
    ...
    body = request.get_json(silent=True) or {}
    # Inject BRAIN credentials from active session for resume
    session_id = request.headers.get('Session-ID') or flask_session.get('brain_session_id')
    if session_id and session_id in brain_sessions:
        sess_info = brain_sessions[session_id]
        if isinstance(sess_info, dict):
            if not body.get('brain_username'):
                body['brain_username'] = sess_info.get('username', '')
            if not body.get('brain_password'):
                body['brain_password'] = sess_info.get('password', '')
...
def _advanced_prompt_sections():
    ...
    return [
        ...
        },
        {
            ...
            'title': '决策阶段系统提示词',
            'description': '控制决策阶段如何挑选下一轮增强目标与增强风格。',
        ...
        },
        {
            ...
            'title': '增强阶段单模板提示词',
            'description': '控制增强阶段针对单个模板做常规增强时使用的系统提示词。若不自定义，新建流水线时系统会在内置 V1/V2 中随机选一个并固化到该流水线。',
        ...
        },
        {
            ...
            'title': '增强阶段交叉增强提示词',
            'description': '控制增强阶段进行交叉融合增强时使用的系统提示词。若不自定义，将保持原有默认提示词，不参与随机切换。',
            'default_text': ENHANCE_SYSTEM_PROMPT_TEMPLATE_V1,
            'locked_tokens': ['{{GUIDE1}}', '{{GUIDE2}}', '{{VECTOR_DATA_HINT}}'],
        },
    ]
...
def _prompt_token_preview(section_key: str, token: str, data_type: str = 'MATRIX', max_enhance_per_round: int = 4):
    ...
    preview_map = {
        ...
        '{{ALLOWED_PLACEHOLDERS_JSON}}': {
            ...
            'content': json.dumps({
                'note': '这里会在运行时替换成 allowed_placeholders 的完整 JSON。它告诉模型哪些 {variable} 可用。',
        ...
        '{{VECTOR_DATA_HINT}}': {
            ...
            'content': vector_hint if str(data_type or 'MATRIX').upper() == 'VECTOR' else '当前 data_type 不是 VECTOR，因此这个变量在运行时会展开为空。',
        },
        '{{VECTOR_OPERATORS_LINE}}': {
            'title': '变量预览：VECTOR_OPERATORS_LINE',
            'description': '当 data_type=VECTOR 时，这里会插入可用向量算子说明；否则为空。',
            'content': vector_operators_line if str(data_type or 'MATRIX').upper() == 'VECTOR' else '当前 data_type 不是 VECTOR，因此这个变量在运行时会展开为空。',
        },
        '{{MAX_ENHANCE_PER_ROUND}}': {
            'title': '变量预览：MAX_ENHANCE_PER_ROUND',
            'description': '决策阶段会把这里替换成你当前创建表单里的“每轮最多增强数”。',
            'content': str(int(max_enhance_per_round or 4)),
        },
        '{{GUIDE1}}': {
            'title': '变量预览：GUIDE1',
            'description': '增强阶段会把“单因子思考逻辑链”指导文档插入到这里。',
            'content': _read_text_optional(trail_dir / 'skills' / 'template_final_enhance' / '单因子思考逻辑链.md') or '未找到指导文档。',
        },
        '{{GUIDE2}}': {
            'title': '变量预览：GUIDE2',
            'description': '增强阶段会把“算子总结”指导文档插入到这里。',
            'content': _read_text_optional(trail_dir / 'skills' / 'template_final_enhance' / 'op总结.md') or '未找到指导文档。',
        },
    }
...
def _enhance_cross_prompt_preview_sections(runner):
    ...
    cross_user = json.dumps({
        'mode': 'cross_enhance',
        'style': 'balanced',
        'instruction': '交叉增强（均衡模式）：在创新性与稳定性之间取得平衡。每个输出应融合至少两个输入模板中有价值的运算符思路，同时保持可实现性和可解释性。',
...
def api_pipeline_update_workers(pipeline_id):
    ...
    if delta > 0 and usage['used'] + delta > usage['limit']:
        return jsonify({'ok': False, 'error': f'全局Worker额度不足 (已用{usage["used"]}/{usage["limit"]})', 'workers': usage}), 400
